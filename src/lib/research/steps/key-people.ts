...
export async function researchKeyPeople(jd: ParsedJD): Promise<KeyPeople> {
  const results = await search(`${jd.companyName} CEO founders leadership team`);

  return generateStructured({
    system: SYSTEM_PROMPTS.keyPeople,
    prompt: buildKeyPeoplePrompt(jd.companyName, jd.roleTitle, jd.department, formatSearchResults(results)),
